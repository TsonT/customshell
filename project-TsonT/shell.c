...
void parse(char* line, command_t* p_cmd) {
    ...
    for(int i = 0; i < argc; i++) {
        strcpy(p_cmd->argv[i], (char *) argv[i]);
    }
...
int find_fullpath( command_t* p_cmd ) {
    ...
    char* temp_path_env_var = malloc(strlen(path_env_variable) + 1); 
    strcpy(temp_path_env_var, path_env_variable);

    folder_path = strtok(temp_path_env_var, PATH_SEPARATOR);

    struct stat buff; 

    while (folder_path != NULL) {
        char* file_path = malloc(strlen(folder_path) + 1);
        strcpy(file_path, folder_path);
        strcat(file_path, "/");
        strcat(file_path, p_cmd->argv[0]);

        if (stat(file_path, &buff) == 0 && (S_IFREG & buff.st_mode)) {
            p_cmd->argv[0] = file_path;
            return TRUE;
        }

        folder_path = strtok(NULL, PATH_SEPARATOR);
    }
